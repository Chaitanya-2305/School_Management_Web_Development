...
export default function AddSchool() {
  ...
  useEffect(() => {
    // This effect runs on the client to check the session status
    ...
    const checkSession = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // If the server-side check fails, redirect to login.
          // We use router.replace to prevent the user from going back to this page.
  ...
  return (
    <div className="max-w-xl mx-auto my-6 p-4 border border-gray-200 rounded-lg shadow-sm bg-white">
      ...
      {message && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
          <div className={`p-6 bg-white rounded-lg shadow-xl text-center max-w-sm ${
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5">
        ...
        <div>
          <label htmlFor="contact" className="block mb-1 font-semibold text-gray-700">
            Contact
          </label>
          <input
            id="contact"
        ...
        <div>
          <label htmlFor="email_id" className="block mb-1 font-semibold text-gray-700">
            Email
          </label>
          <input
            id="email_id"
