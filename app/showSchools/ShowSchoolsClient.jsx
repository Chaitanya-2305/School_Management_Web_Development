...
export default function ShowSchoolsClient() {
  ...
  useEffect(() => {
    const fetchSchools = async () => {
      try {
        ...
        // Ensure each school has a numeric ID
        const schoolsWithId = data.map((s, index) => ({
          ...s,
          id: s.id ?? index + 1
        }));
  ...
  // Sync search & state filters with URL
  useEffect(() => {
  ...
  // Filter schools based on search query & state
  const filteredSchools = schools.filter((school) => {
    const matchesSearch =
      (school.name ?? '').toLowerCase().includes(searchQuery.toLowerCase()) ||
      (school.address ?? '').toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  if (loading) return <div className="text-center mt-12 text-gray-500">Loading schools...</div>;
      ...
      {filteredSchools.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredSchools.map((school, index) => (
            <div
              key={school.id ?? index} // ✅ fallback to index if id missing
              className="bg-white rounded-lg shadow-lg overflow-hidden transform transition-transform duration-300 hover:scale-105"
            >
              <div className="relative w-full h-48">
                {school.image ? (
              ...
              <div className="p-5 flex flex-col justify-between items-center">
                ...
                <button
                  onClick={() => {
                    if (!school.id) return alert('Invalid school ID');
